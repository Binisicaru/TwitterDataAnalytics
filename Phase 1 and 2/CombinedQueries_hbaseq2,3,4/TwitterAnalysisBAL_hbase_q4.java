...
public class TwitterAnalysisBAL{
	...
	TwitterAnalysisBAL(){
		...
		try {
			table = new HTable(config, "hashTagAnalysis");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	// Extract hashtag, starttimestamp,endtimestamp fields from the request.
	...
	public void scanTable() {
		...
		try {
			...
			for(Result result=scanner.next() ; result!=null ; result=scanner.next()) {
				TwitterAnalysisFrontEndService.tweetCache.put(Bytes.toString(result.getRow()), Bytes.toString(result.getValue(Bytes.toBytes("hashtag"), Bytes.toBytes(""))));
				counter++;
				if(counter==299998) {
					break;
				}
